#!/usr/bin/python3
"""
Experimental training with 8 DQN pursuers vs 1 circular motion evader experience collection
"""
import sys
import os
import numpy as np
...
if __name__ == '__main__':
    ...
    for ep in range(num_episodes):
        ...
        obs = env.reset()
        agent_p.linear_epsilon_decay(episode=ep, decay_period=200)
        for st in range(num_steps):
            ...
            if info == "All pursuers deceased":
                ewin_counter += 1
            # log step
            logging.info("\n-\nepisode: {}, step: {}, epsilon: {} \nstate: {} \nactions: {} \nnext_state: {} \nreward: {} \ninfo: {} \npursuers won: {}, evaders won: {}\n-\n".format(ep+1, st+1, agent_p.epsilon, states, actions, next_states, reward, info, pwin_counter, ewin_counter))
            obs = next_obs.copy()
            states = next_states.copy()
            # agent_done = [done[i] for i in range(len(done))]
            agent_done = [d for d in done]
            if info:
                break
        # summarize episode
